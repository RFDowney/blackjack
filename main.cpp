...
bool play(std::vector <char> hand, char dealer, std::vector <char> cardsontable){
   bool ace;
...
/*else if(value(hand)>13 && value(hand)<16 && ace 
&&hand.size()<4
){
return true;
}*/
std::vector <char> dealer_v{};
dealer_v.push_back(dealer);
if(value(dealer_v)>9){
  if(value(hand)<16){
    return true;
  }
